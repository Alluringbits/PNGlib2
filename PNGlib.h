...
namespace PNG{
	...
	class basic_PNG{
		public:
			...
			basic_PNG(const std::string_view filename) noexcept : fn{filename}, bPNGios{nullptr}{/* open(filename)*/};
			virtual const std::unique_ptr<PNGmsg>& open(std::string_view) PNGEXC = 0;
			...
			//Messages vector functions: get last message in the messages list, get the entire message list, empty the messages list
			/*const PNGmsg * lastMsg() noexcept{
				return msgLs.front();
			}
			
			const std::vector<PNGmsg *> msgList() noexcept{
				return msgLs;
			}*/
			...
			const std::unique_ptr<PNGmsg>& lastMsg() noexcept{
			...
			const std::vector<std::unique_ptr<PNGmsg>>& msgList() noexcept{
				if(msgLs.empty()) msgLs.push_back(std::make_unique<noMsg>());
				return msgLs;
			}
			...
			//non constexpr, non noexcept, they are supposed to throw custom PNG PNGmsg s defined in PNGerr.h
		...
		protected:
			...
			basic_PNG(std::string_view f, std::basic_ios<char> *pbs) noexcept : fn{f}, bPNGios{pbs}{}; //protected constructor for the derived classes
			...
			std::vector<std::unique_ptr<PNGmsg>> msgLs{};
			std::unique_ptr<PNGmsg> neutralMsg{std::make_unique<noMsg>()};	
			template<typename T>
			const std::unique_ptr<PNGmsg>&  pngraise(T && e){
				if(PNGTRW) throw(e);
				else{
